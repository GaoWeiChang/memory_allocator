...
static void* bench_worker(void* arg) {
    ...
    for (int i = 0; i < OPS_PER_THREAD; i++) {
        ...
        if (p) {
            ((char*)p)[0] = (char)i;   /* Touch the memory slightly to simulate real-world usage */
            mem_free(p);
        }
    }
    return NULL;
...
int main(void) {
    ...
    printf(" (%d malloc+free per thread, doubling thread count)\n", OPS_PER_THREAD);
    printf("======================================================\n\n");
    ...
    for (int i = 0; i < 4; i++) {
        ...
        double ideal = thread_counts[i];
        printf("  %2d thread(s): Actual speedup = %.2fx  (Ideal scaling = %.2fx)\n",
               thread_counts[i], speedup, ideal);
    }
